...
import DesignCreateEdit from "./components/DesignCreateEdit";
import {
  DesignCreateEditSchema,
  YupDesignCreateEdit,
} from "./yup/DesignCreateEditSchema";

const DesignCreateEditPage: React.FC = () => {
  ...
  const [Design, setDesign] = useState<YupDesignCreateEdit>({
    name: "",
  });
  ...
  const formik = useFormik({
    ...
    validationSchema: DesignCreateEditSchema,
    onSubmit: (values, { resetForm }) => {
      setPageBlocker(true);
      if (!id) {
        DesignService.createDesign(values)
          .then((createdResult) => {
            resetForm({ values });
        ...
        DesignService.updateDesign(id as guid, values as any)
          .then((result) => {
            resetForm({ values });
            setPageBlocker(false);
          ...
          <PbCard px={2} pt={2}>
            ...
            <CardContent>
              <PbTabPanel value={tab} index={0}>
                <DesignCreateEdit formik={formik}></DesignCreateEdit>
              </PbTabPanel>
            </CardContent>
          </PbCard>
        </form>
...
export default DesignCreateEditPage;
