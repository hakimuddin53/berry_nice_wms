...
namespace Wms.Api.Controllers
{
    ...
    public class WarehouseController(IService<Warehouse> service, IMapper autoMapperService) : ControllerBase
    {
        ...
        public async Task<IActionResult> GetSelectOptionsAsync([FromQuery] GlobalSelectFilterV12Dto selectFilterV12Dto)
        {
            ...
            var warehouseDtos = autoMapperService.Map<PagedListDto<SelectOptionV12Dto>>(pagedResult);
        ...
        {
            var warehouses = await service.GetAllAsync(e => e.Name.Contains(warehouseSearch.Search));
            ...
            var warehouseDtos = autoMapperService.Map<PagedListDto<WarehouseDetailsDto>>(pagedResult);         
        ...
        public async Task<IActionResult> CountWarehousesAsync([FromBody] WarehouseSearchDto warehouseSearch)
        {
            ...
            var warehouseDtos = autoMapperService.Map<List<WarehouseDetailsDto>>(warehouses);
        ...
        [HttpGet(Name = "FindWarehouseAsync")]
        public async Task<IActionResult> FindWarehouseAsync([FromQuery] WarehouseFindByParametersDto warehouseFindByParametersDto)
        {
            var warehouseIdsAsString = warehouseFindByParametersDto.WarehouseIds.Select(id => id.ToString()).ToArray();

            var warehousesQuery = await service.GetAllAsync(e => warehouseIdsAsString.Contains(e.Id.ToString()));

            var result = warehousesQuery
                .Skip((warehouseFindByParametersDto.Page - 1) * warehouseFindByParametersDto.PageSize)
                .Take(warehouseFindByParametersDto.PageSize)
                .ToList();

            PagedList<Warehouse> pagedResult = new PagedList<Warehouse>(
                result,
                warehouseFindByParametersDto.Page,
                warehouseFindByParametersDto.PageSize);
