using AutoMapper;
using Wms.Api.Dto;
using Wms.Api.Dto.CartonSize;
using Wms.Api.Entities;

namespace Leitstand.Mapping.Profiles.v12_0;

public class CartonSizeProfile : Profile
{
    public CartonSizeProfile()
    {
        #region DetailsV12Dto
        CreateMap<CartonSize, CartonSizeDetailsDto>(); 
        #endregion 

        #region CreateUpdateV12Dto
        CreateMap<CartonSizeCreateUpdateDto, CartonSize>();
        #endregion

        CreateMap<CartonSize, SelectOptionV12Dto>()
       .ForMember(x => x.Value, option => option.MapFrom(y => y.Id))
       .ForMember(x => x.Label, option => option.MapFrom(y => y.Name));

    }
}